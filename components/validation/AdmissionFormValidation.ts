...
export const appointmentSchema = z.object({
  firstName: z
    ...
    .regex(/^[a-zA-Z\s-]+$/, "First name should only contain letters, spaces, and hyphens"),
  
  lastName: z
    .string()
    .min(1, "Last name is required")
    .regex(/^[a-zA-Z\s-]+$/, "Last name should only contain letters, spaces, and hyphens"),
  ...
  phone: z
    ...
    .regex(/^\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}$/, "Please enter a valid phone number format, e.g. (123) 456-7890"),
  ...
  time: z
    ...
    .refine(validateBusinessHours, {
      message: "Appointments are only available between 9:00 AM and 5:00 PM"
    }),
  
  department: DepartmentEnum, // Uses the enum validation
